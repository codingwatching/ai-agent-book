...
1. **多 Agent**：每个玩家 = 一个独立的 LLM Agent（OpenAI Chat Completions，默认
   `gpt-4o-mini`），各自维护**严格隔离的私有上下文**，只能依据自己看到的信息推理。
...
- **在线模式模型只用 OpenAI**（`OPENAI_API_KEY`，默认 `gpt-4o-mini`）。按实验约束，
  OPENROUTER / ANTHROPIC / DEEPSEEK / SILICONFLOW 的 Key 当前不可用，请勿使用。
