...
Mapping these onto the two IPC paradigms: the shared file system is the Agent world's "shared memory"; tool call parameters and the message bus are two forms of "message passing"—the former delivered synchronously with the call, the latter delivered asynchronously through an intermediary. Each paradigm has its trade-offs. Go has a widely quoted maxim: "Do not communicate by sharing memory; instead, share memory by communicating"—shared memory is fast but leaves the hazards of concurrency conflicts to the user; message passing requires writing more orchestration code but keeps data ownership clear and traceable. This trade-off recurs throughout the later discussions of status queries and concurrency conflicts.
...
Readers familiar with operating systems will recognize this pair of choices: shared context is threads, non-shared context is processes. Threads share an address space, so switching between them is cheap and communication requires no copying, at the cost of no isolation—one thread corrupting memory brings down the whole process. Processes each have their own independent address space, giving thorough isolation and safe parallelism, at the cost of communication having to go through explicit IPC. Every selection criterion in Table 10-1 can be derived from this set of trade-offs.
...
Let's put a definitional dispute on the table first: in the language of Chapter 1, multi-stage role switching is a **workflow-style orchestration**—the execution path (e.g., requirements clarification → implementation → review) is predefined. From the process angle it is even clearer: this is one process executing the code of different stages in turn—what changes is the code segment, while the memory is one and the same throughout, not multiple processes. The view that this is "not really multi-agent" therefore has merit. This chapter still brings it into the multi-agent frame because doing so yields concrete design benefits: when system prompts, tool sets, and focus differ across stages, treating the stages as multiple Agents sharing one trajectory lets each "identity's" prompt and tool set be refined independently, and stage boundaries naturally become quality gates.
...
At the beginning of this chapter, we mapped the communication mechanisms onto the two paradigms of inter-process communication, and mapped shared vs. non-shared context onto threads vs. processes. This analogy can be pushed further (Table 10-3):
...
| Shared memory / message passing | Shared file system / message |

A program is static code; a process is one running instance of a program. Likewise, the static prefix determines who the Agent is, while the trajectory records how far it has progressed. The LLM plays the role of the CPU: it holds no state of its own and time-shares its service among many Agents by loading different contexts—the very term "context switch" was borrowed from operating systems. And for the same reason, swapping in a faster CPU keeps the program running as before; swapping in a stronger model keeps the Agent the same Agent—its identity and memory live in the prefix and the trajectory, not in the model weights.

This abstraction is nothing new: private state, asynchronous messages, and the ability to create new members are precisely the basic setup of the 1970s Actor model[^actor-model], and a multi-agent system may as well be regarded as its LLM edition. As a result, most of the mature experience from operating systems and distributed systems can be borrowed directly. The one place the analogy fails is this: processes pass bytes, bit-for-bit faithful; Agents pass semantics, and every retelling can distort them—this is the new problem taken up specifically in this chapter's "Failure Modes" section.

[^actor-model]: Hewitt, C., Bishop, P., Steiger, R. *A Universal Modular ACTOR Formalism for Artificial Intelligence.* IJCAI 1973.

This process-style isolation brings several practical engineering benefits: each Agent can be developed and tested independently, new capabilities can be added without touching existing code, a failing Agent cannot infect the others with its error state, and multiple Agents can genuinely execute concurrently—contexts are fully independent, with no resource contention.
...
Distributed fault-tolerance theory divides faults into two kinds: **crash faults** (a component stops working) and **Byzantine faults** (a component keeps working but gives out wrong information). Traditional systems mostly only need to guard against crashes; an Agent's faults, however, are Byzantine by nature—it rarely stops outright, but instead keeps producing plausible-looking but wrong conclusions, and the error never announces itself as an error. This explains why patching a single component yields so little: no single component will proactively expose the problem, so it can only be caught through independent redundancy. The cross-validation and majority voting that recur throughout this chapter are exactly the classic means of Byzantine fault tolerance; deterministic external feedback (tests, compilers, database queries) is precious because it is the one component in the system that never lies.
...
Multi-agent systems have two orthogonal core design dimensions: whether context is shared, and how the collaboration topology is organized. Shared context is inheritance-style collaboration—each Agent inherits its predecessor's complete context, losing no information but growing the context fast. Non-shared context is fully independent collaboration, exchanging information through distilled handoff packages, the file system, or message passing. As for topology: the peer collaboration pattern suits iterative refinement among a few Agents, the manager pattern suits complex tasks that need dynamic scheduling, and the decentralized pattern suits scenarios where responsibilities are equal and control must flow among the Agents themselves. All of it rests on two topology-independent infrastructures, whose design blueprint comes from operating systems—an Agent is to the runtime what a process is to the kernel: the static prefix is the program, the trajectory is memory, and the LLM is a time-shared CPU. The data plane is the **shared file system**—in essence a virtual directory tree mounting four kinds of areas (agent-specific workspaces, multi-agent shared spaces, external resources, and built-in system resources), across which Agents exchange artifacts by passing file paths. The control plane is the **communication and control mechanism**, supporting message passing, status queries, execution termination, and resource scheduling. Status queries also fall within the two communication paradigms: either asynchronous question-and-answer via messages, or side-channel observation via the shared file system—reading the trajectory file the sub-agent persists in real time, or reading a lightweight progress file the two sides agree on in advance; the trajectory is the Agent's entire state, and after a crash, loading the trajectory is enough to resume the session. A message bus is a common implementation of the control plane, suited to real-time, asynchronous, multi-party coordination—and once collaboration crosses organizational boundaries, a standardized interoperability protocol like A2A is needed.
